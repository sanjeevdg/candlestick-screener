...
class BotConfig:
    ...
    # Risk management
    stop_loss_atr_mult: float = 2.0    # initial stop = anchor - (ATR * mult)
    take_profit_atr_mult: float = 3.0  # TP   = entry + (ATR * mult)
    trailing_atr_mult: float = 1.5     # trailing stop trails at best_price - (ATR * mult)
...
class SimPosition:
    ...
    best_price: float = 0.0   # high-water mark for trailing stop
...
def compute_trailing_stop(
    ...
    """
    Advance the trailing stop and high-water mark.

    For longs : trail = best_price - ATR * trailing_atr_mult
                Stop only moves UP — never down.
    For shorts: trail = best_price + ATR * trailing_atr_mult
                Stop only moves DOWN — never up.

    Returns (new_stop, new_best_price).
    """
    if direction == "long":
        new_best = max(best_price, current_price)
        new_stop = new_best - atr * cfg.trailing_atr_mult
        return new_stop, new_best
    else:
        new_best = min(best_price, current_price) if best_price != 0.0 else current_price
        new_stop = new_best + atr * cfg.trailing_atr_mult
        return new_stop, new_best
        ...
        while True:
            try:
                ...
                for symbol in cfg.symbols:
                    ...
                    # --- Advance trailing stop, then check exit ---
                    if symbol in open_positions and symbol in position_meta:
                        meta = position_meta[symbol]

                        # Advance trailing stop — stop only moves in the favourable direction
                        trail_stop, new_best = compute_trailing_stop(
                            current_price=current_price,
                            best_price=meta["best_price"],
                            atr=ind.atr,
                            direction=meta["direction"],
                            cfg=cfg,
                        )
                        meta["best_price"] = new_best
                        if meta["direction"] == "long":
                            meta["stop_loss"] = max(meta["stop_loss"], trail_stop)
                        else:
                            meta["stop_loss"] = min(meta["stop_loss"], trail_stop)
                        ...
                        if exit_sig:
                            logger.info(
                                "EXIT SIGNAL | %s | reason=%s | price=%.4f | sl=%.4f",
                                symbol, exit_sig.reason, current_price, meta["stop_loss"],
                            )
                    ...
                    if symbol not in open_positions and len(open_positions) < cfg.max_open_positions:
                        ...
                        if entry_sig:
                            ...
                            position_meta[symbol] = {
                                ...
                                "best_price": current_price,   # high-water mark starts at entry
                            }
    ...
    def run(self) -> None:
        ...
        for i in range(min_bars, n_bars):
            for symbol, full_data in all_data.items():
                ...
                # --- Update trailing stop, then check exit ---
                if symbol in self.state.positions:
                    pos = self.state.positions[symbol]

                    # Advance the trailing stop (stop only moves in the favourable direction)
                    trail_stop, new_best = compute_trailing_stop(
                        current_price=current_price,
                    ...
                    if exit_sig:
                        ...
                        logger.info(
                            "SIM EXIT  | bar=%d | %s | %s | price=%.4f | sl=%.4f | pnl=%.2f | reason=%s",
                            i, symbol, pos.side, current_price, pos.stop_loss, pnl, exit_sig.reason,
                        )
