...
def check_entry(ind: Indicators, cfg: BotConfig) -> Optional[EntrySignal]:
    ...
    if ema_cross_long and ind.rsi < cfg.rsi_overbought:
        sl = price - atr * cfg.stop_loss_atr_mult
...
class SimulationTrader:
    ...
    def _record_trade(self, action: str, symbol: str, qty: float, price: float, reason: str, pnl: float = 0.0) -> None:
    ...
    def run(self) -> None:
            ...
            for symbol, full_data in all_data.items():
                ...
                if symbol in self.state.positions:
                    ...
                    if exit_sig:
                        ...
                        self._record_trade("exit", symbol, pos.qty, current_price, exit_sig.reason, pnl)
        ...
        for symbol, pos in list(self.state.positions.items()):
            ...
            self._record_trade("force_close", symbol, pos.qty, last_price, "end_of_simulation", pnl)
        self.state.positions.clear()

        self._print_summary()

    def _print_summary(self) -> None:
        total_trades = len([t for t in self.state.trade_log if t["action"] == "exit"])
        wins   = [t for t in self.state.trade_log if t["action"] in ("exit", "force_close") and t["pnl"] > 0]
        losses = [t for t in self.state.trade_log if t["action"] in ("exit", "force_close") and t["pnl"] <= 0]
        total_pnl = sum(t["pnl"] for t in self.state.trade_log)
        ...
        logger.info("  Closed trades : %d", total_trades)
        logger.info("  Wins          : %d", len(wins))
        logger.info("  Losses        : %d", len(losses))
        if wins or losses:
            win_rate = 100.0 * len(wins) / max(1, len(wins) + len(losses))
