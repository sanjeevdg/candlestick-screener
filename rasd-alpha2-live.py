...
def check_entry(df):
    ...
    if valid_breakout and strong and vol and trend_ok(df):
    ...
    if vwap_pullback and strong and vol and trend_strength > 0:
...
async def handle_bar(symbol, candle):
    global data, positions, last_entry_price

    log.info(f"BAR: {symbol} {candle['close']}")

    # ===== BUILD ROW FROM CANDLE =====
    new_row = {
        "open": candle["open"],
        "high": candle["high"],
        "low": candle["low"],
    ...
    # ===== APPEND DATA =====
    df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True).tail(200)
    df = add_indicators(df)   # keep if needed
    data[symbol] = df
